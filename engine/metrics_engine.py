...
def main():
    ...
    n=len(matches); players=data.get("players",[]); own_shots=sum(v["for"] for v in shot_counts.values()); opp_shots=sum(v["against"] for v in shot_counts.values())
    metrics={"form":metric("".join(form[:5]) or None,min(5,len(form)),min(5,n),"result-code"),"goals_for":metric(sum(gf) if gf else None,len(gf),n,"goals"),"goals_against":metric(sum(ga) if ga else None,len(ga),n,"goals"),"goals_for_per_match":metric(round(sum(gf)/len(gf),2) if gf else None,n,n,"goals/match","derived" if gf else None),"goals_against_per_match":metric(round(sum(ga)/len(ga),2) if ga else None,n,n,"goals/match","derived" if ga else None),"xg":metric(round(sum(xgf),2) if xgf else None,len(xgf),own_shots,"goals"),"xga":metric(round(sum(xga),2) if xga else None,len(xga),opp_shots,"goals"),"xgot":metric(round(sum(xgotf),2) if xgotf else None,len(xgotf),own_shots,"goals"),"xgot_against":metric(round(sum(xgota),2) if xgota else None,len(xgota),opp_shots,"goals"),"xg_per_match":metric(round(sum(xgf)/n,2) if xgf and n else None,n,n,"goals/match","derived" if xgf else None),"xga_per_match":metric(round(sum(xga)/n,2) if xga and n else None,n,n,"goals/match","derived" if xga else None),"shots_count":metric(len(shots) or None,len(shots),len(shots),"shots"),"shots_with_xg":metric(len(xgf)+len(xga) if xgf or xga else None,len(xgf)+len(xga),len(shots),"shots"),"shots_for":metric(own_shots or None,sum(v["for"]>0 for v in shot_counts.values()),n,"shots"),"shots_against":metric(opp_shots or None,sum(v["against"]>0 for v in shot_counts.values()),n,"shots"),"matches_sample":metric(n,n,n,"matches"),"players_count":metric(len(players) or None,len(players),len(players),"players"),"events_count":metric(len(events) or None,len(events),len(events),"events"),"spatial_actions_count":metric(len(data.get("spatial_actions",[])) or None,len(data.get("spatial_actions",[])),len(data.get("spatial_actions",[])),"actions")}
