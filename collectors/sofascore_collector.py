"""FOSI SofaScore collector: preserve team, squad, matches and match layers."""
import json, time, urllib.parse, urllib.request
from datetime import datetime, timezone
from pathlib import Path
BASE="https://www.sofascore.com/api/v1"; CONFIG=Path("config/selected-scout.json"); ROOT_BASE=Path("data/scouting")
def get_json(path):
...
def resolve_team(name):
    data=get_json("/search/all?q="+urllib.parse.quote(name)); exact=None; fallback=None
    for item in data.get("results",[]):
        ent=item.get("entity") or {}
        if item.get("type")=="team":
            if str(ent.get("name","")).lower()==name.lower(): exact=str(ent.get("id"));break
            if name.lower() in str(ent.get("name","")).lower(): fallback=str(ent.get("id"))
    return exact or fallback,data
def main():
    cfg=json.loads(CONFIG.read_text(encoding="utf-8"))
    if not cfg.get("enabled") or not cfg.get("team"):return
    root=ROOT_BASE/cfg["country"].lower().replace(" ","-")/cfg["competition"].lower().replace(" ","-")/cfg["team_id"];raw=root/"raw"/"sofascore"
    st={"source":"sofascore","status":"collecting","retrieved_at":datetime.now(timezone.utc).isoformat(),"team":cfg["team"],"layers":{},"records":{},"errors":[]}
    try:
        ...
        save(raw/"team.json",get_json(f"/team/{tid}"));st["layers"]["team"]="available";st["records"]["team"]=1
        ...
        for e in events:
            eid=str(e["id"])
            for suffix,key in [("","event"),("/statistics","statistics"),("/incidents","incidents"),("/lineups","lineups"),("/graph","graph"),("/shotmap","shotmap")]:
                try:
                    p=get_json(f"/event/{eid}{suffix}");save(raw/"matches"/eid/("event.json" if not suffix else suffix[1:]+".json"),p);counts[key]+=1
                except Exception as exc:st["errors"].append({"match_id":eid,"layer":key,"error":str(exc)})
                time.sleep(.08)
        st["layers"].update({"events":"available" if counts["incidents"] else "partial","stats":"available" if counts["statistics"] else "partial","spatial":"available" if counts["shotmap"] else "partial"});st["records"].update(counts);st["team_id"]=tid;st["status"]="success" if not st["errors"] else "partial"
