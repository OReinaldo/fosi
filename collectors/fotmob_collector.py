"""FOSI FotMob collector: preserve team, matches and detailed match payloads."""
...
def main():
    ...
    try:
        ...
        if not tid:raise RuntimeError("No FotMob provider id for selected team")
        team=get_json(f"/teams?id={tid}&ccode3=POL");save(raw/"team.json",team);st["layers"].update({"team":"available","players":"available"});st["records"]["team"]=1
        matches={str(m["id"]):m for m in walk_matches(team) if team_match(m,cfg["team"]) and finished(m)}
        matches=sorted(matches.values(),key=lambda m:str((m.get("status") or {}).get("utcTime") or m.get("timeTS") or ""),reverse=True)
        st["layers"]["matches"]="available" if matches else "pending";st["records"]["matches"]=len(matches)
        detail=0;errors=[]
        for m in matches:
            mid=str(m["id"])
        ...
        st["records"]["match_details"]=detail;st["layers"].update({"stats":"available" if detail else "pending","events":"available" if detail else "pending","spatial":"available" if detail else "pending"});st["errors"].extend(errors)
        st["status"]="success" if not errors else "partial"
